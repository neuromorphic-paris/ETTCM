...
do
  ...
  do
    ...
    for r in 2 3 4
    do
      echo "    Resolution reduction multiplier: $r"

      for seq in 2D-1 2D-2 2D-3 2D-4 2D-5 2D-6 2D-7 3D
      do
        echo "      Sequence: $seq"
        ...
        $executable $events $calib $divergence $timestamps -s $s -e 0.01 -m 1000 -l 1 -w $r -g $r
      done
    done
  done
done
